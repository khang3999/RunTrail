...
public interface SpuRepository extends JpaRepository<SpuEntity, Long> {
    ...
    @Query("SELECT DISTINCT new runtrail.dev.backend.dto.response.SpuDTO(u.id, u.spuName, u.spuDescription, u.categoryId, u.brandId, br.brandName, sk.skuPrice, img.imgUrl, u.spuStatus, u.discount, sk.skuAttri) " +
            "FROM SpuEntity u " +
            "INNER JOIN SkuEntity sk ON sk.spu.id = u.id " +
            "INNER JOIN BrandEntity br ON u.brandId = br.id " +
            "INNER JOIN SpuImagesEntity img ON img.spuId = u.id " +
