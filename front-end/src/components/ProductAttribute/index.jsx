...
export function ProductAttributeItem({ attribute }) {
  ...
  const { setData, data, listAttrOutOfStock, setListAttrOutOfStock } = useProductDetailProvider();
...
export function ProductAttributeItemValue({ value, disabled = false, selected, onClick }) {
  if (disabled) {
    return <span className={"inline-block h-[30px] px-3 mb-1  border mx-2 bg-[#eee] text-gray-300"}>{value}</span>
  }
  return (
    <button onClick={onClick} className={`h-[30px] px-3 mb-1 bg-slate-50 border mx-2 ${selected && "border-orange-500 text-orange-500"}`} >{value}</button>
  )
}
    ...
    <ProductAttribute title='Số lượng'>
      <div>
        ...
        <div class="flex items-center rounded border border-gray-200">
          <button disabled={!hiddenQuantity} onClick={() => handleQuantity('minus')} type="button" class={`size-10 leading-10 text-gray-600 transition hover:opacity-75 ${!hiddenQuantity && "bg-gray-50 text-gray-200" }`}>
            &minus;
          </button>
          <input
          ...
          />
          <button disabled={!hiddenQuantity} onClick={() => handleQuantity('plus')} type="button" class={`size-10 leading-10 text-gray-600 transition hover:opacity-75 ${!hiddenQuantity && "bg-gray-50 text-gray-200" }`}>
            &#43;
          </button>
        </div>
      </div>
