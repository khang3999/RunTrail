...
function DropDownSelect({ placeholder, name, value, onChange, label, errorMessage, options, disabled }) {
   return (
      <div className="relative w-full">
         <label htmlFor={name} className="block text-sm font-medium text-gray-700">{label}</label>
         <select
            style={{ appearance: 'none', WebkitAppearance: 'none', MozAppearance: 'none' }} // Hide default select icon
            name={name}
            id={name}
            value={value}
            onChange={(e) => {
         ...
         >
            <option value="default" key={"default"}>{placeholder}</option>
            {options.map(option => (
               <option className="text-black" key={option.key} value={option.key}>{option.value}</option>
            ))}
         </select>
