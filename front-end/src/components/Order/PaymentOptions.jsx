         ...
         <div className="payment-options border rounded-2xl flex flex-col">
            {options.map((option, index) => (
               <button key={index}
                  ...
                  <div>
                     <FontAwesomeIcon icon={payment === option.key ? faDotCircle : faCircle} className={`${payment === option.key ? "text-black" : " text-gray-400"}`} />
                     <label htmlFor={option.key} className={`ml-2 ${payment === option.key ? "text-black" : " text-gray-400"}`}>{option.name}</label>
                  </div>
                  <div>
